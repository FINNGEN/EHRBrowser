...
function App() {
  ...
  async function sendFeedback(text) {
    const response = await fetch('http://127.0.0.1:8563/sendFeedback', {
      method: 'POST',
      headers: {
  ...
  useEffect(()=>{
    ...
    setLoaded(true)
    fetch(`http://127.0.0.1:8563/getAPIInfo`)
      .then(res=> res.json())
      .then(data=>{
        setApiInfo(data)
      })
    fetch(`http://127.0.0.1:8563/getListOfConcepts`)
      .then(res=> res.json())
      .then(data=>{
        setConceptList(data)
  ...
  useEffect(()=>{
    if (root) {
        ...
        fetch(`http://127.0.0.1:8563/getCodeCounts?conceptId=${root}`)
            .then(res=> res.json())
            .then(data=>{
