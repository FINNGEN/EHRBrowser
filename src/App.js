...
function App() {
  ...
  useEffect(()=>{
    if (sidebarRoot) {
      ...
      setMapRoot('')
      const allNodes = sidebarRoot.data.concept_relationships.filter(d => d.levels !== "Mapped from" && d.levels !== "Maps to").filter(d => !levelFilter || (d.levels === '-1' || parseInt(d.levels.split('-')[0]) <= levelFilter))
      ...
      setSelectedConcepts(selected.map(d => ({name:d, leaf: !allNodes.map(d => d.parent_concept_id).includes(d) &&  allNodes.filter(d => d.child_concept_id === d)[0]?.levels !== "-1"? true : false, data: {code_counts: sidebarRoot.data.stratified_code_counts.filter(e => e.concept_id === d), concept: getConceptInfo(d)}})))
      // set nodes and links
