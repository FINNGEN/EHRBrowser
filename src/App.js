...
function App() {
  ...
  const years = []
  for (let i = 1953; i <= 2022; i++) {years.push(i)}

  // fetch data
  async function getData(id) {
    console.log('get',id)
    const res = await fetch(`http://127.0.0.1:8585/getCodeCounts?conceptIds=${id}`)
  ...
  useEffect(()=>{
    if (sidebarRoot) {
      ...
      // setMapRoot(sidebarRoot.name)
      setMapRoot('')
      //ISSUE **45557625 does not have root relationship */
      const allNodes = sidebarRoot.data.concept_relationships.filter(d => d.levels !== "Mapped from" && d.levels !== "Maps to")
      const selected = allNodes.filter(d => d.levels !== '-1' && getConceptInfo(d.child_concept_id).record_counts !== 0).map(d => d.child_concept_id).filter((e,n,l) => l.indexOf(e) === n)
      ...
      setSelectedConcepts(selected.map(d => ({name:d, data: {code_counts: sidebarRoot.data.stratified_code_counts.filter(e => e.concept_id === d), concept: getConceptInfo(d)}})))
      // set nodes and links
      let listArray = nodeList.map(e=>({
          ...
          'total_counts': getConceptInfo(e).record_counts,
          'descendant_counts': getConceptInfo(e).descendant_record_counts,
      ...
      listArray = listArray.map(node => ({
          ...
          mappings: mappings.filter(d => d.parent_concept_id === node.name && getConceptInfo(d.child_concept_id).concept_name).map(e=>({
              ...
              'total_counts': getConceptInfo(e.child_concept_id).record_counts,
              'descendant_counts': getConceptInfo(e.child_concept_id).descendant_record_counts,
