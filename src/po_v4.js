...
const po = {
    ...
    createPoset:(input, elementNames = null) => {
        ...
        function coloringLogic(layer,iNeg,i,delta){
            if(iNeg===0){
                ...
                // ****** EDIT 
                const categories = circularSOM(roots.map(r=>r[1]),roots.map(r=>r[0]))
                ...
                // const rootsCategorized =  categories.neurons.filter(neuron=>neuron.bmus.length>0)
                //     .map(neuron=>(neuron.ref
                //         .map(ref=>(angle=categories.toHue(neuron.position),theta=angle*Math.PI/180,{
                //             "id":ref,
                //             "theta":angle,
                //             "x":Math.cos(theta),
                //             "y":Math.sin(theta)
        ...
        function color(delta=1,lThreshold=40,hThreshold=80,flip=false){
            
            poset.setDepth()
            
            const {coloringLogic} = poset
            poset.climber(poset,coloringLogic,[delta])
            ...
            poset.feature("fill",(node)=>{
                ...
                // ****** EDIT
                // console.log(d.depth,poset.layers.length,d.depth/poset.layers.length)
