...
function YearFilter({ minYear, maxYear, yearSelection, setYearSelection, width = 600 }) {
  ...
  useEffect(() => {
    ...
    function attachHandleDrag(handle, which) {
      handle.call(
        d3.drag().on("drag", (event) => {
          ...
          let [lo, hi] = selectionRef.current || [minYear, maxYear];
          ...
          const [lo, hi] = selectionRef.current;
          if (lo === minYear && hi === maxYear) {
            setYearSelectionRef.current(null);
          } else {
            setYearSelectionRef.current(selectionRef.current);
          }
        })
      );
    }
    ...
    if (selectionRef.current) {
      updateVisuals(selectionRef.current[0], selectionRef.current[1]);
    } else {
      updateVisuals(minYear, maxYear);
    }
  ...
  useEffect(() => {
    ...
    if (yearSelection) {
      ...
      c.updateVisuals(minYear, maxYear);
    }
  }, [yearSelection, minYear, maxYear]);
