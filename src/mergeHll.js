...
function decodeBlob(input) {
  ...
  if (typeof input === 'string') {
    ...
    // Handles Buffer (in Node), Uint16Array, DataView, etc.
    ...
    throw new Error(
      'input must be a base64 string, Uint8Array, ArrayBuffer, or TypedArray'
    );
  }
...
function normalizeSketches(input) {
  ...
  return arr
    ...
    .map((x) => {
      ...
      throw new Error(
        "sketch must be a base64 string, Uint8Array, ArrayBuffer, or TypedArray"
      );
    });
}
