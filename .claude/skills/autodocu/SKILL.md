...
`Tests/run_test.sh` runs **every** section's spec in one shot (it does not filter to a single section): it starts the app, waits for it, runs all specs, diffs the screenshots, stops the app, and writes a report. It works from any directory and always tears the app down, even on failure. It accepts an optional passthrough filter (`./run_test.sh 1.some_section`) and the `--accept-changes` flag (below); the two can be combined (`./run_test.sh --accept-changes 1.some_section`). On start it sources `Tests/.env` for tunable settings (currently `AUTODOCU_DIFF_THRESHOLD`, the changed-pixel % below) and `Tests/Scripts/app_control.sh` for the app-specific start/stop.
...
- changed pixels **> 1%** → a real, visible change, marked **FAIL**. By **default** the committed image is **kept** and the old + new images are saved as a BEFORE/AFTER pair under `Tests/test_report/` for review (never silently overwritten). With **`--accept-changes`** the committed image is instead **replaced** with the fresh capture and marked **PASS** — adopting it as the future baseline.
...
The output is `Tests/test_report.md`: one **bold** row per section carrying the Playwright pass/fail, one plain row per screenshot carrying its diff pass/fail, and a final **`**Total**`** row (e.g. `2/5`). When screenshots changed and were not accepted, the report also gets a **Visual changes to review** section — grouped by section then screenshot — embedding the BEFORE/AFTER images (from `Tests/test_report/`) side by side. The script's exit code is 0 only if every row passed.
...
- **`Scripts/compare_report.mjs` is generic too** — a plain Node script (stdlib `fs` + `zlib`, no dependencies, no Python) that decodes the PNGs, computes the changed-pixel %, and writes `test_report.md`. For over-threshold screenshots it saves the BEFORE/AFTER pair to `test_report/` and writes the visual-review section by default, or replaces the baseline when `run_test.sh` passes its accept flag. Shipped by the template, copied on `init`; never customized per app.
