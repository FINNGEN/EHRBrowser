...
- Committed sources: `Tests/README.md`, `Tests/run_test.sh`, `Tests/compare_report.mjs`, `Tests/app_control.sh`, `Tests/test_report.md`, each `Tests/<section>/<slug>.spec.ts`, and under `Tests/playwright/`: `package.json`, `package-lock.json`, `playwright.config.ts`, `_helpers.ts`, `.gitignore`. Everything else is ignored.

`Tests/README.md` documents this layout for humans — the template ships it.

### The one-command test runner: `run_test.sh` + `compare_report.mjs` + `app_control.sh`

`Tests/run_test.sh` runs **every** section's spec in one shot (it does not filter to a single section): it starts the app, waits for it, runs all specs, diffs the screenshots, stops the app, and writes a report. It works from any directory and always tears the app down, even on failure. It accepts an optional passthrough filter (`./run_test.sh 1.some_section`).

**Screenshots are diffed, not blindly overwritten.** Fresh captures go to a temp folder first — the specs write there because `run_test.sh` exports `AUTODOCU_SHOT_ROOT` and `_helpers.ts`'s `shot()` honours it. Then `compare_report.mjs` pixel-compares each capture against the committed image in `Documentation/<section>/screenshots/`. A pixel counts as *changed* only if a channel differs by more than ±16/255 (`PIXEL_DELTA_TOLERANCE`), so the invisible sub-pixel wobble that anti-aliased regions — gradients, filled charts — show between runs is ignored; without that tolerance a large chart flags ~9% "changed" while looking identical. Then:
...
- **`compare_report.mjs` is generic too** — a plain Node script (stdlib `fs` + `zlib`, no dependencies, no Python) that decodes the PNGs, computes the changed-pixel %, replaces baselines over threshold, and writes `test_report.md`. Shipped by the template, copied on `init`; never customized per app.
